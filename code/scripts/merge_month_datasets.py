...
def _load_year_population(sex_file_name: str) -> pd.Series:
    sex_path = OUTPUT_DIR / sex_file_name
    sex_df = pd.read_csv(
        sex_path,
        dtype={YEAR_COL: "string", YEAR_CODE_COL: "string"},
    )
...
def _load_age_population(age_file_names: list[str], filter_combined: bool = False) -> pd.DataFrame:
...
def _calculate_monthly_aamr(
    ...
    age_population = _load_age_population(age_file_names, filter_combined)
...
def _fill_population_and_rates(
    ...
    if AAMR_COL in out.columns:
        out[AAMR_COL] = out[f"{AAMR_COL}_calculated"]
        out = out.drop(columns=f"{AAMR_COL}_calculated")
    return out
